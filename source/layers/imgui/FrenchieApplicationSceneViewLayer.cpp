...
namespace Frenchie
{
    namespace Application
    {
        ...
        class ObjectScreenShoter : public Frenchie::Core::Component
        {
        public:
            
            ObjectScreenShoter(){}

            virtual ~ObjectScreenShoter()
            {
                if(m_Framebuffer != 0)
            ...
            {
                ImGui::Text("Framebuffer %s", std::to_string(m_Framebuffer));
                ImGui::SameLine();
                ImGui::Text("TextureColorBuffer %s", std::to_string(m_TextureColorBuffer));
                ImGui::Text("TextureDepthBuffer %s", std::to_string(m_TextureDepthBuffer));
            }
...
void SceneView::frame_update()
{
    ...
    if(camera == nullptr || size == nullptr)
        return;
...
{
    auto size = m_Scene != nullptr ? m_Scene->get_component<Size>() : nullptr;
