    ...
    const gs_mat4f& _Transform)
{
    ...
    // build mesh
    build_line_mesh(_P1, _P2, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_line_mesh(_P2, _P3, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_line_mesh(_P3, _P1, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    ...
    const gs_mat4f& _Transform)
{
    ...
    build_line_mesh(_P1, _P2, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_line_mesh(_P2, _P3, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_line_mesh(_P3, _P4, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_line_mesh(_P4, _P1, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    ...
    const gs_mat4f& _Transform)
{
    ...
    // sides
    build_arc_mesh(TL, radius, radius, 90.f, 180.f, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_arc_mesh(BL, radius, radius, 180.f, 270.f, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_arc_mesh(TR, radius, radius, 0.f, 90.f, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_arc_mesh(BR, radius, radius, 270.f, 360.f, _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_line_mesh(arc_point(TL, radius, radius, 180), arc_point(BL, radius, radius, 180), _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_line_mesh(arc_point(TL, radius, radius, 90), arc_point(TR, radius, radius, 90), _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_line_mesh(arc_point(TR, radius, radius, 0), arc_point(BR, radius, radius, 0), _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);
    build_line_mesh(arc_point(BL, radius, radius, 270), arc_point(BR, radius, radius, 270), _Depth, _LineWidth, m_DefaultTexture, _Color, m_Vertexes, m_Indexes);

    push_rendering_command(m_DefaultTexture, _Color, _Transform);
}

gs_rectf Immediate2DRenderer::calculate_text_bounding_box(
    const std::string&        _Text,
    const float&              _Size,
    const gs_vec2f&           _Position,
    const gs_mat4f&           _Transform,
    const RenderingQueueFont& _Font)
{
    ...
    height = gs_max(height, gs_abs(positionY)) - font.Descent * scale;
    width  = gs_max(width, positionX);

    gs_vec2f transformedPosition = _Transform * gs_vec4f(_Position, 1.f, 1.f);
    ...
    std::vector<int>&                  _Indexes)
{
    ...
    {
        p2 = gs_vec2f(_Center.x + _MinorRadius * cos(gs_to_radians(angle)), _Center.y + _MajorRadius * sin(gs_to_radians(angle)));
        build_line_mesh(p1, p2, _LineWidth, _LineWidth, _Texture, _Color, _Vertexes, _Indexes);
    }
}