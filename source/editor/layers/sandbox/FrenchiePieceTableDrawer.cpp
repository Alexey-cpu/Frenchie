...
void PieceTableDrawer::frame_update()
{
    ImGui::Begin(get_name().c_str());
    {
        ...
        ImGui::BeginChild("Text");
        {
            ...
            int source = m_Table->get_line_start_index(start);
            int target = m_Table->get_line_start_index(end);

			Frenchie::Core::PieceTableSymbolIterator b(m_Table.get(), source);
			Frenchie::Core::PieceTableSymbolIterator e(m_Table.get(), target);
