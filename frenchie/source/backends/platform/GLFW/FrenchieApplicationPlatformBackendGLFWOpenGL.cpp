...
{
    return std::string(glfwGetWindowTitle(reinterpret_cast<GLFWwindow*>(m_Api->Window)));
}

std::string ApplicationPlatformBackend::get_clipboard_text()
{
    return std::string(glfwGetClipboardString(reinterpret_cast<GLFWwindow*>(m_Api->Window)));
}

bool ApplicationPlatformBackend::has_clipboard_text()
{
    return glfwGetClipboardString(reinterpret_cast<GLFWwindow*>(m_Api->Window)) != nullptr;
}

void ApplicationPlatformBackend::set_window_name(const std::string& _Value)
{
    glfwSetWindowTitle(reinterpret_cast<GLFWwindow*>(m_Api->Window), _Value.c_str());
}
