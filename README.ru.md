...
Frenchie - **это компактный C++ framework для разработки приложений с графическим интерфейсом пользователя**. Основная цель проекта - создать простой в использовании инструмент, позволяющий быстро создавать приложения с графическим интерфейсом пользователя на языке C++ для множества платформ.

## **Начало работы**

### **Требования к набору инструментов**

**Frenchie** является C++ библиотекой, которая использует систему сборки CMake. Для того, чтобы начать работать с **Frenchie**, нужно установить компилятор с поддержкой C++ 17 и выше, а также CMake 3.3 или новее.

### **Как это работает**

Приложение на **Frenchie** - это бесконечный цикл, который обрабатывает список слоев приложения до тех пор, пока приложение не закроется. Каждый слой реализует ограниченный набор функций. Для запуска контекстного окна и рисования графики, Frenchie использует системный и графический backend-ы. Системный backend представляет из себя слой абстракции над платформо-зависимым функционалом операционной системы, при помощи которого открывается окно и  отслеживаются его события. Графический backend - это слой абстракции над графическим API, отвечающий за загрузку геометрии и текстур на видеокарту для последующей рисовки.

### **Пример создания простого проекта**

При создании **CMake** проекта рекомендуется использовать модуль **CMake FetchContent** для скачивания и конфигурирования актуальной версии **Frenchie**. Предлагается использовать такую структура папок:
...
На данный момент, **Frenchie** использует сторонние библиотеки в качестве системного backend-а. Ниже представлен список сторонних библиотек, которые можно использовать в качестве системного backend-а **Frenchie**:
...
Для создания простого приложения достаточно включить файл **frenchie.cmake** в **CMakeLists.txt** файл проекта, добавить библиотеку в проект **Frenchie** через поддиректорию, и далее собрать список файлов с исходным кодом и используемых директорий проекта и передать его в функцию **СMake**, которая создает проект исполняемого файла приложения. Ниже представлен код, который выполняет вышеуказанные процедуры:
...
```
Для создания списка файлов с исходным кодом и подключаемый к проекту диреторий, **Frenchie** предоставляет удобный **СMake** макрос, расположенный в файле **collect_source_code_and_resources.cmake**. Данный макрос умеет собирать в список одиночные C/C++ и Objective-C/C++ файлы, а также искать эти файлы в заданных директориях.

Далее, чтобы начать разрабатывать приложение, требуется создать слой, который будет добавлять слой UI в цикл приложения. Рекомендуется для каждого отдельного окна приложения создавать отдельный слой. Ниже представлен код, который сосздает простое окно с редактором цвета:
